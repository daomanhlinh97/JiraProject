...
public class PullDataFromJira {
	...
	public void pullDataProject() throws IOException{
        ...
        String encoding = Base64.getEncoder().encodeToString("daomanhlinh97:thangngu".getBytes("utf-8"));
	...
	@PostConstruct
	public void pullDataEmployee() throws IOException, JSONException, ParseException{
        ...
        int sbstart = chuoi.indexOf("[");
        int sbend = chuoi.indexOf("]");
        String chuoiMoi = chuoi.substring(sbstart, sbend+1);
      
        InputStream stream = new ByteArrayInputStream(chuoiMoi.getBytes(StandardCharsets.UTF_8));
        
        InputStreamReader isr = new InputStreamReader(stream);

        TypeToken<List<Employee>> token = new TypeToken<List<Employee>>(){};
        List<Employee> list = new Gson().fromJson(isr, token.getType());
        for (int i=0;i<list.size();i++)
        {
